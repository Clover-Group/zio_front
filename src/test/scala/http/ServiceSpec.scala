...
import clover.tsp.front.http.TodoService.TodoItemWithUri
...
class TodoServiceSpec extends HTTPSpec {
  ...
  describe("Working TodoService") {

    ignore("should create new todo items") {
    ...
    ignore("should delete all todo items") {
    ...
    ignore("should update todo items") {
      val setupReq  = request[TodoTask](Method.POST, "/").withEntity(TodoItemPostForm("Test"))
  ...
  describe("Failing TodoService") {

    ignore("should list all todo items") {
      val setupReq  = request[TodoTask](Method.POST, "/").withEntity(TodoItemPostForm("Test"))
      val req       = request[TodoTask](Method.GET, "/")
      runWithEnv(
        check[TodoTask, List[TodoItemWithUri]](
...
object TodoServiceSpec extends DefaultRuntime {
  ...
  val todoService: TodoService[Repository] = TodoService[Repository]("")
